...
check_requirement "Server file exists" \
    "[ -f $PAI_CONFIG_DIR/VoiceServer/server.ts ]" \
    "Server file missing at \$PAI_CONFIG_DIR/VoiceServer/server.ts"
...
if [ -z "$PORT_CHECK" ]; then
    ...
    echo "   Stop the conflicting service or change PORT in ~/.config/assistant/.env"
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
fi
...
# Check ~/.config/assistant/.env file exists
if [ -f ~/.config/assistant/.env ]; then
    echo -e "${GREEN}✅ ~/.config/assistant/.env file exists${NC}"
    
    # Check for ElevenLabs API key
    if grep -q "ELEVENLABS_API_KEY=" ~/.config/assistant/.env 2>/dev/null; then
        # Check if it's not a placeholder
        API_KEY=$(grep "ELEVENLABS_API_KEY=" ~/.config/assistant/.env | cut -d'=' -f2)
        if [[ "$API_KEY" == "your_api_key_here" ]] || [[ -z "$API_KEY" ]]; then
            ...
            if grep -q "ELEVENLABS_VOICE_ID=" ~/.config/assistant/.env 2>/dev/null; then
        ...
        check_warning "ElevenLabs API key configured" \
            "false" \
            "Add ELEVENLABS_API_KEY=your_key to ~/.config/assistant/.env for AI voices"
    fi
else
    check_warning "~/.config/assistant/.env file exists" \
        "false" \
        "Create ~/.config/assistant/.env and add ELEVENLABS_API_KEY=your_key for AI voices"
fi
    ...
    if launchctl list | grep -q "com.paivoice.server"; then
        ...
        if [ "$PID" != "-" ]; then
            ...
            if curl -s http://localhost:8888/health > /dev/null 2>&1; then
                ...
                echo "   Check logs: tail -f \$PAI_CONFIG_DIR/VoiceServer/logs/voice-server-error.log"
                ISSUES_FOUND=$((ISSUES_FOUND + 1))
            fi
        else

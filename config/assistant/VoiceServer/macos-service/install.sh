...
if [ -f ~/.config/assistant/.env ] && grep -q "ELEVENLABS_API_KEY" ~/.config/assistant/.env 2>/dev/null; then
    echo "✅ ElevenLabs API key found in ~/.config/assistant/.env"
else
    ...
    echo "   2. Add to ~/.config/assistant/.env file:"
    ...
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Installation cancelled. Set up ~/.config/assistant/.env and try again."
        exit 1
    fi
fi

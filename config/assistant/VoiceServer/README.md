...
Add your ElevenLabs API key to `~/.config/assistant/.env`:
```bash
echo "ELEVENLABS_API_KEY=your_api_key_here" >> ~/.config/assistant/.env
echo "ELEVENLABS_VOICE_ID=s3TPKV1kjDlVtZbl4Ksh" >> ~/.config/assistant/.env
```
...
```bash
cd ~/.config/assistant/voice-server
./install.sh
```
...
```bash
cd ~/.config/assistant/voice-server/menubar
./install-menubar.sh
```
...
Create or edit `~/.config/assistant/.env` in your home directory:
...
4. Update `ELEVENLABS_VOICE_ID` in your `~/.config/assistant/.env`
...
grep ELEVENLABS_API_KEY ~/.config/assistant/.env
...
- **No hardcoded API keys**: All sensitive data is read from `~/.config/assistant/.env`

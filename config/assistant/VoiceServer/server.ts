...
// Load .env from PAI directory (PAI_DIR env var, or ~/.config/assistant, or ~/.claude fallback)
const paiDir = process.env.PAI_DIR || join(homedir(), '.config', 'assistant');
...
try {
  const corePersonalitiesPath = join(paiDir, 'skills', 'CORE', 'SYSTEM', 'AGENTPERSONALITIES.md');
...
try {
  const pronunciationsPath = join(paiDir, 'skills', 'CORE', 'USER', 'pronunciations.json');

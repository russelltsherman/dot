...
`~/.config/assistant/skills/CORE/USER/SKILLCUSTOMIZATIONS/Browser/`
...
**Headless by default.** All automation runs in headless mode. When the user says "show me" or wants to see what the assistant is seeing, open the URL in their preferred browser from `~/.config/assistant/skills/CORE/USER/TECHSTACKPREFERENCES.md`:
...
bun run ~/.config/assistant/skills/Browser/Tools/Browse.ts https://example.com
...
**Location:** `~/.config/assistant/skills/Browser/Tools/Browse.ts`
...
**Location:** `~/.config/assistant/skills/Browser/Tools/BrowserSession.ts`
...
```typescript
import { PlaywrightBrowser } from '~/.config/assistant/skills/Browser/index.ts'

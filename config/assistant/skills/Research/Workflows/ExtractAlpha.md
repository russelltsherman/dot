...
```bash
~/.config/assistant/MEMORY/WORK/{current_work}/scratch/
```

**To get the current work directory:**
1. Read `~/.config/assistant/MEMORY/STATE/current-work.json`
2. Extract the `work_dir` value
3. Use `~/.config/assistant/MEMORY/WORK/{work_dir}/scratch/` for temporary artifacts
...
```
~/.config/assistant/MEMORY/WORK/20260111-172408_extract-alpha-analysis/scratch/
├── raw-transcript.txt
...
```bash
~/.config/assistant/History/research/YYYY-MM-DD_description/
```
...
```
~/.config/assistant/History/research/2025-10-26_podcast-analysis/
...
1. **Check if hooks captured the output:**
   ...
   ls -lt ~/.config/assistant/History/research/ | head -5

   # Verify your research directory exists
   ls ~/.config/assistant/History/research/YYYY-MM-DD_description/
   ```

2. **If hooks did NOT capture automatically:**
   ```bash
   # Create directory structure manually
   mkdir -p ~/.config/assistant/History/research/YYYY-MM-DD_description/
...
3. **Confirm all files saved:**
   ```bash
   ls -lah ~/.config/assistant/History/research/YYYY-MM-DD_description/
...
WORK_DIR=$(jq -r '.work_dir' ~/.config/assistant/MEMORY/STATE/current-work.json)

# 2. Create scratch workspace in current work item
mkdir -p ~/.config/assistant/MEMORY/WORK/${WORK_DIR}/scratch/
cd ~/.config/assistant/MEMORY/WORK/${WORK_DIR}/scratch/
...
mkdir -p ~/.config/assistant/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
ls -lah ~/.config/assistant/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
2. **Get current work directory** - Read `~/.config/assistant/MEMORY/STATE/current-work.json` for `work_dir`
3. **Create scratch workspace** - Work in `~/.config/assistant/MEMORY/WORK/{work_dir}/scratch/`
...
6. **Save to history** - Final outputs to `~/.config/assistant/History/research/YYYY-MM-DD_description/`

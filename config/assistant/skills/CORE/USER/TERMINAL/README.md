...
3. **Copy configuration:**
   ...
   cp ~/.config/assistant/skills/CORE/USER/TERMINAL/kitty.conf ~/.config/kitty/
   ```

4. **Add shell aliases:**
   Add the contents of `ZSHRC` to your `~/.zshrc`:
   ```bash
   cat ~/.config/assistant/skills/CORE/USER/TERMINAL/ZSHRC >> ~/.zshrc
   source ~/.zshrc
   ```
...
**Voice not working:**
- Voice server must be running (`~/.config/assistant/VoiceServer/`)

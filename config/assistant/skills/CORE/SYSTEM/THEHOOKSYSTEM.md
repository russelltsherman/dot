...
**Location:** `~/.config/assistant/hooks/`
**Configuration:** `~/.config/assistant/settings.json`
...
- **History Capture** - Automatic work/learning documentation to `~/.config/assistant/MEMORY/`
...
Claude Code supports the following hook events (from `~/.config/assistant/hooks/lib/observability.ts`):
...
- Writes to `~/.config/assistant/MEMORY/SIGNALS/ratings.jsonl`
...
- After receiving Task result, parent calls: `bun ~/.config/assistant/skills/CORE/Tools/UpdateTabTitle.ts "Message"`
- This avoids transcript parsing race conditions
- See `~/.config/assistant/skills/CORE/Tools/UpdateTabTitle.ts` for implementation
...
### Location
**File:** `~/.config/assistant/settings.json`
**Section:** `"hooks": { ... }`

### Environment Variables
Hooks have access to all environment variables from `~/.config/assistant/settings.json` `"env"` section:
...
**Session Mapping:** `~/.config/assistant/MEMORY/STATE/agent-sessions.json`
```json
{
...
**Full Documentation:** See `~/.config/assistant/skills/CORE/SYSTEM/TERMINALTABS.md`
...
**Location:** `~/.config/assistant/hooks/my-custom-hook.ts`
...
```bash
chmod +x ~/.config/assistant/hooks/my-custom-hook.ts
```
...
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.config/assistant/hooks/my-custom-hook.ts
```
...
**Check:**
1. Is hook script executable? `chmod +x ~/.config/assistant/hooks/my-hook.ts`
2. Is path correct in settings.json? Use `${PAI_DIR}/hooks/...`
3. Is settings.json valid JSON? `jq . ~/.config/assistant/settings.json`
4. Did you restart Claude Code after editing settings.json?

**Debug:**
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.config/assistant/hooks/my-hook.ts

# Check hook logs (stderr output)
tail -f ~/.config/assistant/hooks/debug.log  # If you add logging
```
...
**Check:**
1. Does `~/.config/assistant/MEMORY/` directory exist?
2. Is AutoWorkCreation hook running? Check `~/.config/assistant/MEMORY/STATE/current-work.json`
3. Is hook actually running? Check `~/.config/assistant/MEMORY/RAW/` for events
4. File permissions? `ls -la ~/.config/assistant/MEMORY/WORK/`

**Debug:**
```bash
# Check current work
cat ~/.config/assistant/MEMORY/STATE/current-work.json

# Check recent work directories
ls -lt ~/.config/assistant/MEMORY/WORK/ | head -10
ls -lt ~/.config/assistant/MEMORY/LEARNING/$(date +%Y-%m)/ | head -10

# Check raw events
tail ~/.config/assistant/MEMORY/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
```
...
grep '"event_type":"Stop"' ~/.config/assistant/MEMORY/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
# Result: 0 matches (no Stop events)

# But other hooks ARE working
grep '"event_type":"PostToolUse"' ~/.config/assistant/MEMORY/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
...
1. **Added CAPTURE field to response format** (see `~/.config/assistant/skills/CORE/SKILL.md`)
...
3. **Verification Commands:**
   ...
   cat ~/.config/assistant/MEMORY/STATE/current-work.json
   ls -lt ~/.config/assistant/MEMORY/WORK/ | head -5
   ls -lt ~/.config/assistant/MEMORY/LEARNING/$(date +%Y-%m)/ | head -5
...
**Tracking:** Documented in `~/.config/assistant/skills/CORE/SKILL.md` (History Capture System section)
...
**Check:**
1. Is `~/.config/assistant/MEMORY/STATE/agent-sessions.json` writable?
...
cat ~/.config/assistant/MEMORY/STATE/agent-sessions.json | jq .

# Check subagent-stop debug log
tail -f ~/.config/assistant/hooks/subagent-stop-debug.log
```
...
cd ~/.config/assistant/skills/system/observability/dashboard/apps/server
bun run dev
...
grep '"type":"user"' ~/.config/assistant/projects/-Users-daniel--claude/*.jsonl | head -1 | jq '.type'
...
**Check:**
1. Does `~/.config/assistant/skills/CORE/SKILL.md` exist?
...
bun ~/.config/assistant/hooks/LoadContext.hook.ts
...
- Permission denied → `chmod +x ~/.config/assistant/hooks/LoadContext.hook.ts`
...
- **Voice System:** `~/.config/assistant/VoiceServer/SKILL.md`
- **Agent System:** `~/.config/assistant/skills/CORE/SYSTEM/AGENTPERSONALITIES.md`
- **History/Memory:** `~/.config/assistant/skills/CORE/SYSTEM/MEMORYSYSTEM.md`
- **Observability Dashboard:** `~/.config/assistant/Observability/`
...
KEY FILES:
~/.config/assistant/settings.json              Hook configuration (daidentity, principal, env vars)
~/.config/assistant/hooks/                     Hook scripts
~/.config/assistant/hooks/lib/identity.ts      Identity loader (reads from settings.json)
~/.config/assistant/hooks/lib/observability.ts Observability helper library
~/.config/assistant/hooks/lib/learning-utils.ts Learning categorization (SYSTEM/ALGORITHM)
~/.config/assistant/hooks/lib/time.ts      PST timestamp utilities
~/.config/assistant/MEMORY/RAW/                Event logs (JSONL) - source of truth
~/.config/assistant/MEMORY/WORK/               Primary work tracking (work directories)
~/.config/assistant/MEMORY/LEARNING/           Learning captures (SYSTEM/, ALGORITHM/, SIGNALS/)
~/.config/assistant/MEMORY/STATE/              Runtime state (current-work.json, progress/, etc.)
~/.config/assistant/MEMORY/STATE/agent-sessions.json  Session→Agent mapping
...
Path: ~/.config/assistant/skills/CORE/Tools/Inference.ts

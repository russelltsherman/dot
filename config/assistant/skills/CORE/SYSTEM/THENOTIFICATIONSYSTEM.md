...
**State file:** `~/.config/assistant/current-effort.json` stores current effort for downstream hooks.
...
**Full voice registry:** `~/.config/assistant/skills/CORE/SYSTEM/AGENTPERSONALITIES.md`
...
Located in `~/.config/assistant/settings.json`:
...
The notification service is in `~/.config/assistant/hooks/lib/notifications.ts`:

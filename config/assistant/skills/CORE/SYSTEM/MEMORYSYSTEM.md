...
**Location:** `~/.config/assistant/MEMORY/`
...
**Location:** `~/.config/assistant/projects/-Users-{username}--claude/`
...
ls ~/.config/assistant/MEMORY/WORK/ | tail -5
```

### Check ratings
```bash
tail ~/.config/assistant/MEMORY/LEARNING/SIGNALS/ratings.jsonl
```
...
ls -lt ~/.config/assistant/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail ~/.config/assistant/projects/-Users-{username}--claude/$(ls -t ~/.config/assistant/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```

### Check learnings
```bash
ls ~/.config/assistant/MEMORY/LEARNING/SYSTEM/
ls ~/.config/assistant/MEMORY/LEARNING/ALGORITHM/
ls ~/.config/assistant/MEMORY/LEARNING/SYNTHESIS/
```

### Check failures
```bash
# List recent failure captures
ls -lt ~/.config/assistant/MEMORY/LEARNING/FAILURES/$(date +%Y-%m)/ 2>/dev/null | head -10

# View a specific failure
cat ~/.config/assistant/MEMORY/LEARNING/FAILURES/2026-01/*/CONTEXT.md | head -100

# Migrate historical low ratings to FAILURES
bun run ~/.config/assistant/skills/CORE/Tools/FailureCapture.ts --migrate
```

### Check multi-session progress
```bash
ls ~/.config/assistant/MEMORY/STATE/progress/
```

### Run harvesting tools
```bash
# Harvest learnings from recent sessions
bun run ~/.config/assistant/skills/CORE/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ~/.config/assistant/skills/CORE/Tools/LearningPatternSynthesis.ts --week
```
...
- Moved BACKUPS to `~/.config/assistant/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration
- Previous: `~/.config/assistant/history/`, `~/.config/assistant/context/`, `~/.config/assistant/progress/`
- Current: `~/.config/assistant/MEMORY/`

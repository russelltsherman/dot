...
function getStats(): SystemStats {
  ...
  try {
    const settings = JSON.parse(readFileSync(join(PAI_CONFIG_DIR, "settings.json"), "utf-8"));
  ...
  try {
    const getCountsPath = join(PAI_CONFIG_DIR, "skills/CORE/Tools/GetCounts.ts");
  ...
  try {
    const historyFile = join(PAI_CONFIG_DIR, "history.jsonl");

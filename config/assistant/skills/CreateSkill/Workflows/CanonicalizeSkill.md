...
```bash
~/.config/assistant/skills/[skill-name]/SKILL.md
```
...
```bash
cp -r ~/.config/assistant/skills/[skill-name]/ ~/.config/assistant/History/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```

**Note:** Backups go to `~/.config/assistant/History/Backups/`, NEVER inside skill directories.
...
cd ~/.config/assistant/skills/[SkillName]/Workflows/
...
find ~/.config/assistant/skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```

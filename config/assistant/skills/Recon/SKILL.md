...
`~/.config/assistant/skills/CORE/USER/SKILLCUSTOMIZATIONS/Recon/`
...
- `naabu` - Port scanning
- Note: Requires security MCP profile (`~/.config/assistant/MCPs/swap-mcp security`)
...
Reports saved to:
- **Work scratch/** (`~/.config/assistant/MEMORY/WORK/{current_work}/scratch/`) - For iterative artifacts during investigation
- **MEMORY/RESEARCH/** (`~/.config/assistant/MEMORY/RESEARCH/YYYY-MM/`) - For pentest engagements and formal assessments
...
~/.config/assistant/MCPs/swap-mcp security
...
Assistant: Activating recon skill with passive-recon workflow.
     ...
     Report: ~/.config/assistant/MEMORY/WORK/{current_work}/scratch/recon-example-com/
```
...
**Security Skills:**
- `~/.config/assistant/skills/OSINT/` - Entity and people reconnaissance
- `~/.config/assistant/skills/Webassessment/` - Web application testing

...
// Paths
const HOME = homedir();
const PAI_CONFIG_DIR = process.env.PAI_CONFIG_DIR || process.env.HOME + '/.config/assistant';
const ZSHRC = join(HOME, '.zshrc');
const VOICE_SERVER_DIR = join(PAI_CONFIG_DIR, 'VoiceServer');
...
function setupZshAlias(): void {
  const aliasLine = `alias pai='bun ~/.config/assistant/skills/CORE/Tools/pai.ts'`;
  ...
  return {
    ...
    "permissions": {
      ...
      "ask": [
        ...
        "Bash(rm -rf ~)", "Bash(rm -rf ~:*)", "Bash(rm -rf ~/.config/assistant)", "Bash(rm -rf ~/.config/assistant:*)",
        ...
        "Read(~/.gnupg/private*)", "Write(~/.config/assistant/settings.json)",
        "Edit(~/.config/assistant/settings.json)", "Write(~/.ssh/*)", "Edit(~/.ssh/*)"
      ],
      "defaultMode": "default"
    },
...
function validate(): { passed: boolean; results: string[] } {
  ...
  // Check settings.json
  const settingsPath = join(PAI_CONFIG_DIR, 'settings.json');
  ...
  for (const dir of ['skills', 'MEMORY', 'hooks']) {
    if (existsSync(join(PAI_CONFIG_DIR, dir))) {
  ...
  // Check CORE skill
  if (existsSync(join(PAI_CONFIG_DIR, 'skills', 'CORE', 'SKILL.md'))) {
...
async function main(): Promise<void> {
  ...
  if (ELEVENLABS_KEY) {
    ...
    // Write API key to ~/.config/assistant/.env
    const ENV_PATH = join(PAI_CONFIG_DIR, '.env');
    ...
    printSuccess('Saved ElevenLabs API key to ~/.config/assistant/.env');
  } else {
  ...
  writeFileSync(join(PAI_CONFIG_DIR, 'settings.json'), JSON.stringify(settings, null, 2));
  ...
  for (const dir of requiredDirs) {
    const dirPath = join(PAI_CONFIG_DIR, dir);
  ...
  if (config.ELEVENLABS_KEY) {
    ...
    printInfo('To enable later: add ELEVENLABS_API_KEY to ~/.config/assistant/.env');
  }
